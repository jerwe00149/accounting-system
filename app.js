...
// === Server-based persistence config (MUST be declared before seedPayroll IIFE
//     because saveData() reads _useServer, and the IIFE may call saveData()) ===
...
// (API_BASE and _useServer hoisted to the top of the file — see note above
//  near STORE_KEY. Do not redeclare here.)
...
async function loadDataFromServer(){
  ...
  try{
    ...
    if(resp.ok){
      ...
      refreshAll();
      // Hash routing after data loaded
...
function parseContractText(text){
  ...
  // 案名 (案名)
  const caseMatch=full.match(/案\s*名[：:]\s*(.+?)(?:\r?\n)/);
  if(caseMatch) d.projectName=caseMatch[1].trim();

  // 業務編號 (案號)
  const caseNoMatch=full.match(/(?:案\s*號|合約編號|契約編號)[：:]\s*(\S+)/);
  if(caseNoMatch){d.businessNo=caseNoMatch[1].trim();d.year=d.businessNo.length>=3?d.businessNo.slice(0,3):'';}
